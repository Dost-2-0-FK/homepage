...
def transform(player, distribution): 
    ...
    player_ctx["attributes"]["cur_char"] = char["attributes"]["key"]
    ...
    diary_ctx["attributes"]["cur_char"] = char["attributes"]["key"]
    diary_ctx["attributes"]["cur_username"] = char["attributes"]["username"]


    print("PLAYER: ", json.dump(player_ctx))
    print("DIARY: ", json.dump(diary_ctx))
    inp = input("Safe? (y/n): ")
    if inp != "y":
        exit("aborted.")
    ...
    # Also update and store char: 
    char["attributes"]["player"] = player_key
    with open(PATH_TO_TXTAD_CHARS.joinpath(f"{chars[0]}.ctx"), 'w') as f:
        json.dump(char, f)
...
if __name__ == "__main__": 
    ...
    for file in PATH_TO_DOST_PLAYERS.glob("*.json"):
        with open(file, "r") as f:
            players.append(transform(json.load(f), distribution))
