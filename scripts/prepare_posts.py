...
PATH_TO_POSTS_TEMPLATE = Path("resources/private_posts.json")

PATH_TO_PLEROMA_POSTS_JSON = Path("pleroma/posts.json")
...
def prepare_posts(post_template, chars): 
    ...
    mentions = post_template["mentions"] 

    mentioned_usernames = []
    # Attributes 
    for attribute, value in mentions["attributes"].items(): 
        for (char, ctx) in chars: 
            if ctx["attributes"][attribute] == value: 
